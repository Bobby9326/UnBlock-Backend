...
| Pre-deploy command | `npm run prisma:deploy && npm run seed`       |
...
- **Migrations and seeding run automatically on every deploy** via the
  pre-deploy command. Both are idempotent: `prisma:deploy` applies only pending
  migrations; `seed` skips if the super admin already exists.
...
| `SEED_ADMIN_EMAIL` / `SEED_ADMIN_PASSWORD` | first super admin's credentials    |

> The seed **refuses to create an admin with the built-in default password when
> `NODE_ENV=production`** — you must set `SEED_ADMIN_PASSWORD` to a real secret,
> or the first deploy will start with no admin (a warning is logged, the deploy
> still succeeds). Set it before the first deploy.
